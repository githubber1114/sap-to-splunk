...
class http_event_collector:
    ...
    def requests_retry_session(self, retries=3,backoff_factor=0.3,status_forcelist=(408,500,502,503,504),session=None):
        ...
        # modified the following line to remove method_whitelist which was throwing the error:
        #      TypeError: Retry.__init__() got an unexpected keyword argument 'method_whitelist'
        # retry = Retry(total=retries, read=retries, connect=retries, backoff_factor=backoff_factor, status_forcelist=status_forcelist, =frozenset(['HEAD', 'TRACE', 'GET', 'PUT', 'OPTIONS', 'DELETE', 'POST']))
    ...
    def server_uri(self):
       ...
       # Build and set server_uri for http event collector
        ...
        # the following line after the comment, modified for troubleshooting purposes
        # server_uri = '%s://%s:%s/services/collector%s' % (protocol, self.http_event_server, self.http_event_port, input_url)
        server_uri = '%s://%s/services/collector%s' % (protocol, self.http_event_server, input_url)
        return (server_uri)
        ...
        try:
            ...
            if response:
                ...
                if response.status_code in acceptable_status_codes:
                    self.log.info("Splunk Server URI is reachable.")
                    # following line commented out DH
